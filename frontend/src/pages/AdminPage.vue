  ...
  <q-page class="column full-height">
    ...
    <q-tab-panels v-model="selectedTab" animated class="col">
        ...
        <div class="q-gutter-md">
          <q-card>
            <q-card-section class="bg-grey-1 text-primary">
              <div class="text-subtitle1">Import Questions</div>
            </q-card-section>
            <q-card-section>
              <q-uploader
          ...
          <q-card>
            <q-card-section class="bg-grey-1 text-primary">
              <div class="text-subtitle1">ColorVote</div>
            </q-card-section>
          ...
          <q-card v-if="comStore.room?.questions?.length">
            <q-card-section class="bg-grey-1 text-primary">
              <div class="text-subtitle1">Current Questions</div>
            </q-card-section>
            <q-list bordered separator>
              <q-item v-for="(question, index) in comStore.room.questions" :key="index">
                <q-item-section avatar>
                  <q-avatar text-color="primary" size="md"> {{ index + 1 }}. </q-avatar>
                </q-item-section>
                <q-item-section>
                  <q-item-label lines="1">{{ stripHtml(question.content) }}</q-item-label>
                </q-item-section>
                <q-item-section side> ({{ question.answers.length }}) </q-item-section>
              </q-item>
            </q-list>
          </q-card>
        </div>
      ...
      <q-tab-panel name="poll">
        <div class="q-gutter-md">
          <q-card v-if="comStore.room">
            ...
            <q-card-section class="row justify-between">
              <q-btn
                ...
                color="primary"
                flat
                :disable="comStore.room.state === SocketEvents.RoomState.LOBBY"
                @click="comStore.previousState()"
              />
              <q-btn
              ...
              />
              <q-btn label="Next" color="primary" flat @click="comStore.nextState()" />
            </q-card-section>
          </q-card>
          ...
          <div
            v-if="
              isQuestionState &&
              comStore.questionIndex !== undefined &&
      ...
      <q-tab-panel name="admin">
        <q-card>
          <q-card-section class="bg-grey-1 text-primary">
            <div class="text-subtitle1">Cleanup</div>
          </q-card-section>
          <q-card-section>
            <q-btn label="Close Room" color="warning" flat @click="confirmClose" />
          </q-card-section>
        </q-card>
      </q-tab-panel>
    </q-tab-panels>
...
watch(
  ...
  (newState) => {
    if (newState) {
      selectedTab.value =
        newState === SocketEvents.RoomState.LOBBY ? 'questions' : 'poll'
    }
  },
)
...
const stateOptions = computed(() => {
  const options = [{ label: 'Lobby', value: SocketEvents.RoomState.LOBBY }]
  if (comStore.room?.questions) {
    comStore.room.questions.forEach((q, index) => {
      options.push({
        label: `${index + 1}. ${stripHtml(q.content)}`,
        value: `q${index}`,
      })
    })
  }
  options.push({ label: 'Results', value: SocketEvents.RoomState.RESULTS })
  return options
})
...
onMounted(() => {
  ...
  if (comStore.room?.state) {
    selectedTab.value =
      comStore.room.state === SocketEvents.RoomState.LOBBY ? 'questions' : 'poll'
  }
})
